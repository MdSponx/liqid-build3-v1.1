          ...
          <div className="relative group">
            ...
            <div className="absolute right-full top-0 mr-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 hidden group-hover:block">
              <div className="py-1">
                ...
                >
                  <FileDown size={16} className="mr-3 text-gray-500 dark:text-gray-400" />
                  PDF Document (A4 + Thai)
                </button>
                <button
                  onClick={() => {
                    ...
                    // Import and use the DOCX export function
                    import('../../utils/docxExport').then(({ exportToDocx }) => {
                      exportToDocx(
                        blocks,
                        header.title || 'Untitled Screenplay',
                        header.author || 'LiQid Screenplay Writer',
                        header.contact || ''
                      );
                    }).catch(error => {
                      console.error('Error importing DOCX export:', error);
                      alert('Failed to load DOCX export module. Please try again.');
                    });
                    onClose();
                  }}
                  className="flex items-center w-full px-3 py-2 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700"
