...
const SceneHeadingSuggestionsImproved: React.FC<SceneHeadingSuggestionsImprovedProps> = ({
  ...
  const [savingStatus, setSavingStatus] = useState<{id: string, status: 'saving' | 'success' | 'error'} | null>(null);
  ...
  const getStatusIndicator = (suggestion: SuggestionItem) => {
    // Check if this suggestion is currently being saved
    if (savingStatus && savingStatus.id === suggestion.label.replace(/\s+/g, '-').toLowerCase()) {
      if (savingStatus.status === 'saving') {
        return (
          <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />
        );
      } else if (savingStatus.status === 'success') {
        return (
          <span className="px-2 py-0.5 bg-green-500 text-white text-xs font-semibold rounded-full animate-pulse">
            SAVED
          </span>
        );
      } else if (savingStatus.status === 'error') {
        return (
