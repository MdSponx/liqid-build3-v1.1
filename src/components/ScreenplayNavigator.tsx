...
const ScreenplayNavigator: React.FC<ScreenplayNavigatorProps> = ({
  ...
  const handleExportPDF = () => {
    // Get screenplay blocks from the editor
    const blocks = window.screenplay?.state?.blocks;
    if (!blocks || !Array.isArray(blocks)) {
      console.error('Could not access screenplay blocks data');
      alert('Could not access screenplay data. Please try again.');
      return;
    }
    ...
    import('../utils/pdfExport').then(({ exportToPDF }) => {
      ...
      console.error('Error importing PDF export:', error);
      alert('Failed to load PDF export module. Please try again.');
    });
  };

  const handleExportDocx = () => {
    // Get screenplay state from the editor
    const editorElement = document.querySelector('[data-screenplay-editor="true"]');
    if (!editorElement) {
      console.error('Could not find screenplay editor');
      return;
    }
    ...
    const blocks = window.screenplay?.state?.blocks;
    if (!blocks || !Array.isArray(blocks)) {
      console.error('Could not find screenplay blocks');
      alert('Could not access screenplay data. Please try again.');
      return;
    }
        ...
        <div className="h-16 flex items-center justify-between border-b border-[#577B92]/20">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-all duration-200 flex items-center bg-[#1E4D3A] text-white hover:bg-[#1E4D3A]/90`}
            >
